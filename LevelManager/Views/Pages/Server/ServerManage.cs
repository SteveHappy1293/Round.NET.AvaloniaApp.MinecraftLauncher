...
public class MinecraftServerDiscoverer
{
    private const int MinecraftLanPort = 4445;
    private const int DiscoveryTimeout = 2000;  

    private const string DiscoveryMessage =
        "M-SEARCH * HTTP/1.1\r\nHOST: 239.255.255.250:1900\r\nMAN: \"ssdp:discover\"\r\nST: urn:schemas-microsoft-com:service:ContentDirectory:1\r\nMX: 3\r\n\r\n";

    public static List<ServerEntry> DiscoverMinecraftServers(IPAddress networkAddress)
    {
        var servers = new List<ServerEntry>();
        var localEndPoint = new IPEndPoint(networkAddress, 0);

        using (var udpClient = new UdpClient(localEndPoint))
        {
            udpClient.EnableBroadcast = true;
            udpClient.Client.ReceiveTimeout = DiscoveryTimeout;

            // Send discovery message to the multicast address
            ...
            udpClient.Send(discoveryBytes, discoveryBytes.Length, multicastEndPoint);
            ...
            while ((DateTime.Now - startTime).TotalMilliseconds < DiscoveryTimeout)
            {
                try
                {
                    ...
                    Message.Show(remoteEndPoint.Address.ToString(), response, InfoBarSeverity.Error);

                        string serverName = ExtractServerName(response);
    ...
    private static string ExtractServerName(string response)
    {
        int motdIndex = response.IndexOf("MOTD=");
        if (motdIndex >= 0)
        {
            int endIndex = response.IndexOf('\n', motdIndex);
            if (endIndex == -1) endIndex = response.Length;

            string motdLine = response.Substring(motdIndex, endIndex - motdIndex);
            string[] parts = motdLine.Split('=');
            if (parts.Length >= 2)
            {
                return parts[1].Trim();
            }
