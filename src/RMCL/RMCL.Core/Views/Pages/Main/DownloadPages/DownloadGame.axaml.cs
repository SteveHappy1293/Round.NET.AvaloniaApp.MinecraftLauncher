...
using RMCL.Core.Views.Windows.Main.DownloadWindows;
            ...
            if (string.IsNullOrEmpty(searchText))
            {
                Versions.ForEach(x =>
                {
                    Dispatcher.UIThread.Invoke(() =>
                    {
                        ...
                        it.OnDownload = ((id) =>
                        {
                            new DownloadClient(x).ShowDialog(Models.Classes.Core.MainWindow);
                        });
            ...
            else
            {
                ...
                filteredVersions.ForEach(version =>
                {
                    Dispatcher.UIThread.Invoke(() =>
                    {
                        ...
                        it.OnDownload = ((id) =>
                        {
                            new DownloadClient(version).ShowDialog(Models.Classes.Core.MainWindow);
                        });
                        VersionsList.Children.Add(it);
                    });
                });
