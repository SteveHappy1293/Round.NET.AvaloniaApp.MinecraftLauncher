...
public class AvaloniaResourcesPool : IDisposable
{
    ...
    public AvaloniaResourcesPool(TimeSpan inactiveTimeout, TimeSpan cleanupInterval)
    {
    ...
    public Bitmap GetImage(string imagePath,int width = 0)
    {
        ...
        // 加载新图片
        var image = width != 0 ? Bitmap.DecodeToWidth(AssetLoader.Open(new Uri(imagePath)), 24) : new Bitmap(AssetLoader.Open(new Uri(imagePath)));
