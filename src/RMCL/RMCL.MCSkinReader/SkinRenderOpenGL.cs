...
public partial class SkinRenderOpenGL(OpenGLApi gl) : SkinRender
{
    ...
    public unsafe void OpenGlRender(int fb)
    {
        ...
        if (_renderType == SkinRenderType.MSAA && _msaaFrameBuffer != 0)
        {
            BindMSAAFramebuffer();
        }
        else
        {
            // 直接使用传入的fb，确保它是有效的
            gl.BindFramebuffer(gl.GL_FRAMEBUFFER, fb >= 0 ? fb : 0);
        }
    ...
    private void BindMSAAFramebuffer()
    {
        ...
        {
            Console.WriteLine($"MSAA帧缓冲不完整: {status}");
            gl.BindFramebuffer(gl.GL_FRAMEBUFFER, 0);
        }
    }
