...
using RMCL.Core.Views.Pages.Main.DownloadPages.DownloadAssets;
...
public partial class DownloadCurseForgeAssets : UserControl
{
    ...
    private CurseForgeAssetsItem GetItem(ModInfo info)
    {
        ...
        it.BtnOnClick = modInfo => Models.Classes.Core.ChildFrame.Show(new DownloadAssetsDetailsPage(info));
        return it;
    }
    public DownloadCurseForgeAssets()
    {
        ...
        Task.Run(() =>
        {
            ...
            catch
            {
                Dispatcher.UIThread.Invoke(() =>
                {
                    Models.Classes.Core.MessageShowBox.AddInfoBar("无法搜索", "无法搜索 CurseForge 资源，可能您未连接互联网", InfoBarSeverity.Error);
                    SearchConfigBox.IsEnabled = true;
                    AssetsList.IsVisible = false;
                    LoadingBox.IsVisible = false;
